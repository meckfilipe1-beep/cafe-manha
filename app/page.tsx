...
const DIAS_FUNCIONAMENTO = [
  {valor:"seg", nome:"Segunda"}, {valor:"ter", nome:"Terça"}, {valor:"qua", nome:"Quarta"},
  {valor:"qui", nome:"Quinta"}, {valor:"sex", nome:"Sexta"}, {valor:"sab", nome:"Sábado"}, {valor:"dom", nome:"Domingo"}
]



// 🆕 NOVO: Horários disponíveis (mesmos que você já tem)
const HORARIOS_DISPONIVEIS = [
  ...
  const unsubscribeLoja = onSnapshot(refLoja, (snap) => {
    if (snap.exists()) {
...
// ✅ BUSCA TUDO DO BANCO: DISPONIBILIDADE, PREÇO, NOME E ÍCONE
...
const unsubscribeProdutos = onSnapshot(qProdutos, (snap) => {
  ...
  // ✅ CORRIGIDO: Agora pega o PREÇO DO BANCO para calcular tudo!
  let subtotal = 0
  let qtdComidas = 0
 ...
 function irParaConferencia(e: React.FormEvent) {
  ...
  async function processarEnvioPedido() {
    ...
    setStatusPix("normal")
    setCodigoPix("")
    // ✅ DEPOIS QUE ENVIAR O PEDIDO, LIMPA TUDO PARA O PRÓXIMO
...
setMostrarListaHorarios(false)
  ...
  async function salvarPedidoNoBanco() {
    ...
    const payloadPedido = {
      ...
      const irParaConferencia = (e) => {
  e.preventDefault()

  // ✅ VALIDAÇÃO AQUI, DEPOIS DE TODO O RESTO
  if (!nome.trim()) { setErroValidacao("Digite seu nome!"); return }
  if (!endereco.trim()) { setErroValidacao("Digite o endereço!"); return }
  if (!numeroCasa.trim()) { setErroValidacao("Digite o número!"); return }
 if (!diaEscolhido) {
  setErroValidacao("⚠️ ESCOLHA O DIA DA ENTREGA!");
  document.getElementById("campo-horario")?.scrollIntoView({ behavior: "smooth" });
  return;
}
  if (!horario) { setErroValidacao("Escolha o HORÁRIO da entrega!"); document.getElementById("campo-horario")?.scrollIntoView({behavior:"smooth"}); return }
  if (!pagamento) { setErroValidacao("Escolha a forma de pagamento!"); return }
  if (pagamento === "Dinheiro" && !trocoPara.trim()) { setErroValidacao("Informe o valor para troco ou deixe 0 se não precisar!"); return }

  // Se tudo estiver certo, vai para a confirmação
  setEtapa("confirmacao")
}
    try {
 ...
 if (!lojaAberta) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFAF5] flex flex-col items-center justify-center px-4 text-center">
      <div 
        className="max-w-md w-full bg-[#FFFFFF] border-4 border-orange-500 rounded-3xl p-8 shadow-2xl space-y-6"
        // ✅ BORDA LARANJA DESTAQUE ADICIONADA ACIMA
      >
  ...
  <p className="text-lg text-[#030314]">
    Olá,
  </p>
...
<p className="bg-red-50 border-2 border-red-500 text-red-700 p-3 rounded-2xl font-black text-sm uppercase shadow-inner text-center">
  ...
  <div className="max-w-2xl mx-auto px-4 mt-6 space-y-4">
    {/* ✅ AVISO DO COMBO: TAMANHO BOM, COR DA LETRA BEM FORTE */}
...
<form onSubmit={irParaConferencia} className="space-y-3 text-[11px] relative" noValidate>
  {/* ✅ ALERTA DE ERRO NA FRENTE DE TUDO */}
  {erroValidacao && (
  ...
  <div className="bg-[#FFFFFF] border border-[#F3F4F6]/80 p-4 rounded-2xl space-y-3 shadow-md">
    ...
    {/* ==== FORMA DE PAGAMENTO (CENTRALIZADA + CORES FORTES) ==== */}
    <div className="bg-[#FFFFFF] border border-[#F3F4F6]/80 p-4 rounded-2xl space-y-3 shadow-md">
      <div>
